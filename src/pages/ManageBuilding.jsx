...
const ManageBuilding = () => {
  ...
  useEffect(() => {
    if (selectedBuildingId) {
      ...
      if (building) {
        ...
        // Get linked images and combine with direct images
  ...
  const selectAllPdfBuildings = () => {
    const allBuildingIds = buildings.filter(b => b.id !== selectedBuildingId).map(b => b.id);
  ...
  const handleDeletePdf = async (pdfId) => {
    try {
      ...
      const currentBuilding = updatedBuildings.find(b => b.id === selectedBuildingId);
      if (currentBuilding) {
        const linkedPDFs = getLinkedPDFs();
        const allPdfs = [
        ...
        {activeTab === 'floors-risers' && (
          <div>
            ...
            {selectedBuildingId && (
              <>
                <Card className="mockup-card mb-4">
                  <h3 className="mockup-card-header">Add Floor</h3>
                  <form onSubmit={handleAddFloor}>
                    ...
                    <Button type="submit" className="btn-mockup mt-2">Add Floor</Button>
                  </form>
                </Card>

                <Card className="mockup-card mb-4">
                  <h3 className="mockup-card-header">Add Riser</h3>
                  <form onSubmit={handleAddRiser}>
                    ...
                    <Button type="submit" className="btn-mockup mt-2">Add Riser</Button>
                  </form>
                </Card>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {images.map(image => (
                    <Card key={image.id} className="mockup-card flex flex-col items-center">
                      <img
                        src={image.url}
                        alt={`${image.type} for ${buildings.find(b => b.id === selectedBuildingId)?.name}`}
